...
class JobController extends Controller
{
    ...
    {
        $records = Job::where('assigned_to', auth()->user()->id)->orderBy('status', 'asc')->paginate('20');
        return view('tr.freelancer.jobs', ['jobs' => $records]);
    }
    ...
    public function edit(Request $request)
    {
        ...
        $job = Job::where('id', $request->id)->where('assigned_to', $user->id)->first();
        ...
        try {
            if (auth()->user()->can('list GstInfo')) {
                return view('tr.freelancer.view_job', compact('job', 'job_city'));
            } else {
    ...
    public function update_job(Request $request)
    {
        ...
        $job = Job::where('id', $job_id)->where('assigned_to', $user->id)->first();
        $update_data = [];
